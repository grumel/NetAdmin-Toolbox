...
export function initialize(container) {
  ...
  let matches = STANDARD_PORTS;
  input.addEventListener("input", () => {
    matches = searchPorts(input.value);
    body.innerHTML = rows(matches);
    empty.hidden = matches.length > 0;
  });
  container.querySelector("#port-export").addEventListener("click", () => exportCSV("port-search", ["Port", "Protocol", "Service", "Description"], matches.map(({ port, protocol, service, description }) => [port, protocol, service, description])));
}
